...
process_vmx_files() {
  ...
  while IFS='' read -r -u9 line || [[ -n "$line" ]]; do
    ...
    if [ "$update_vmx_uuid" = "y" ]; then
      ...
      read -p "No UUID.ACTION.     Add (c for copy (create) / m for move (keep) / n for no)? [n]: " update_vmx_uuid
      if [ "$update_vmx_uuid" = "c" ]; then
        echo -e "\r" >> "$line"
        echo -e "# Added by procvm.sh\r" >> "$line"
        echo -e "uuid.action = \"create\"\r"  >> "$line"
      fi
      if [ "$update_vmx_uuid" = "m" ]; then
        echo -e "\r" >> "$line"
        echo -e "# Added by procvm.sh\r" >> "$line"
        echo -e "uuid.action = \"keep\"\r"  >> "$line"
      fi
    fi
...
show_menus() {
  ...
  echo
  echo "7. Generate labmgr vmx import file"
  echo "8. Generate labmgr vmx import file with snapshot"
  echo
  echo "9. Define search root"
  echo "10. Define path to VMware"
  echo "11. Exit"
  echo
}
...
read_options() {
  ...
  case $choice in
    ...
    7|gli) generate_labmgr_vmx_import_file ;;
    8|glis) generate_labmgr_vmx_import_file_with_snapshot ;;

    9|sp) define_search_root ;;
    10|vp) define_vmware_path ;;
    11) echo
    my_exit ;;
    ?|h) display_readme ;;
