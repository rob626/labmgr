...
class Global_defaults_model extends CI_Model {
	 ...
	 */
	public function get_global_default($global_defaults_id) {
		$q = "SELECT * FROM global_defaults where global_defaults_id = ?";
		$result = $this->db->query($q, $global_defaults_id);
		return $result->result_array();
	}
	     ...
	     */
	    public function update_global_defaults($global_defaults_id, $global_defaults_name, $global_defaults_val) {
	    	$this->db->trans_start();

	    	$this->db->where('global_defaults_id', $global_defaults_id);
	    ...
	    public function add_global_defaults($global_defaults_name, $global_defaults_val) {
	    	...
	    	if(isset($result[0]['global_defaults_id'])) {
	    		return $result[0]['global_defaults_id'];
	    	} else {
	    		$data = array(
					'name' => $global_defaults_name,
					'value' => $global_defaults_val
	     ...
	     */
	    public function delete_global_default($global_defaults_id) {
	    	$this->db->trans_start();
	    	$this->db->where('global_defaults_id', $global_defaults_id);
